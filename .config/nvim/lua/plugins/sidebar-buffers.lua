...
local function get_buffers(ctx)
    ...
    for _, buffer in ipairs(vim.api.nvim_list_bufs()) do
        if buffer ~= view.View.bufnr then
            ...
            -- Ignore unlisted buffers: LSP diagnostics create phantom
            -- buffers via bufadd() for every file with a diagnostic.
            -- Session-restored buffers (badd) ARE listed, so they stay.
